...
10. **DON'T** write PR titles as sentences or pad PR bodies - match the repo's own PRs (#2-#12): the title is a noun phrase ending in a noun (`fix: 할인률을 전체 패키지 최댓값으로 계산 및 0%일 때 문구 숨김`), and the body is a few clean lines - a handful of bullets a reviewer can skim. No `## 개요` / `## 범위` / `## 검증` sections, no tables, no bold emphasis. Investigation notes and measurements belong in the commit message, not the PR body
